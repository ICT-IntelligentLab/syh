function x_est = OTFS_mp_detector(N, M, M_mod, taps, delay_taps, Doppler_taps, chan_coef, noiseVar, y)
    ...
    p_map = ones(N*M, taps, M_mod_ext) * (1/M_mod_ext);
    ...
    for ite = 1:n_ite
            ...
            for ele2 = 1:N
                ...
                for tap_no = 1:taps
                    ...
                    if ele1 - 1 < delay_taps(tap_no)
                        n_idx = mod(ele2 - 1 - Doppler_taps(tap_no), N) + 1;
                        add_term1 = exp(-1i*2*pi*((n_idx-1)/N));
                    end
                ...
                var_int_sum = sum(var_int_hat) + noiseVar;
        ...
        sum_prob_comp = zeros(N*M, M_mod_ext);
        dum_eff_ele1 = zeros(taps,1);
        dum_eff_ele2 = zeros(taps,1);

        for ele1 = 1:M
            for ele2 = 1:N
                dum_sum_prob = zeros(M_mod_ext,1);
                log_te_var = zeros(taps, M_mod_ext);
                ...
                for tap_no = 1:taps
                    ...
                    for i2 = 1:M_mod_ext
                        idx_y = N*(eff_ele1-1) + eff_ele2;
                        dum_sum_prob(i2) = abs(yv(idx_y) - mean_int(idx_y, tap_no) - new_chan*alphabet(i2))^2;
                        dum_sum_prob(i2) = -(dum_sum_prob(i2) / var_int(idx_y, tap_no));
                    end
                ...
                for tap_no = 1:taps
                    ...
                    idx_map = N*(eff_ele1-1) + eff_ele2;
                    p_map(idx_map, tap_no, :) = ...
                        (dum_sum/dum1) * delta_fra + ...
                        (1-delta_fra) * reshape(p_map(idx_map, tap_no, :), 1, M_mod_ext);
                end
            end
        end
