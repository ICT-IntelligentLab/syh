    ...
    methods
        function this = LteScrambler(rnti, cellId, frameNo, codeword) % Constructor
            if nargin > 0
                ...
                % this.hInt2Bit = int2bit('BitsPerInteger', 31);
                ...
                % 直接调用 int2bit 函数，生成 31 位二进制向量（MSB 优先）
                this.initialStates1 = int2bit(c_init(1), 31, true);
                this.initialStates2 = int2bit(c_init(2), 31, true);
            end
        end
